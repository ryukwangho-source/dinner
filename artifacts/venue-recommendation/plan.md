...
| 안내 문구(토스트/배너) | ~~별도 토스트 라이브러리 없이 인라인 배너~~ → **Task 4 구현 중 번복: `sonner`(`toast()`) 사용** | shadcn 스킬의 Critical Rule("Toast via sonner. Use toast() from sonner.")을 뒤늦게 확인 — 이 프로젝트의 `components/ui/*`는 shadcn 규칙을 항상 따라야 하므로, 이 표의 최초 결정보다 shadcn 규칙이 우선한다. 저장·삭제·공유처럼 일회성 확인 알림은 `toast()`로, "예산에 맞는 장소가 없어요"처럼 결과 화면에 계속 붙어있는 문구는 기존대로 `Alert` 컴포넌트로 — 성격이 다른 두 안내를 하나의 방식으로 억지로 통일하지 않는다 |
...
- **수용 기준**:
  - [x] 지역 드롭다운 옵션은 `config/venues.ts`에 있는 지역만 나온다 (자유 텍스트 입력 없음)
  - [x] 인원수·예산을 비운 채 "추천받기" 클릭 → 각 필드 아래 "입력해주세요"가 표시되고 화면 전환은 없다
  - [x] 지역="강남역", 인원수=8, 예산=30000 입력 후 "추천받기" 클릭 → `/results?region=강남역&people=8&budget=30000`로 이동한다
  - [x] 홈 화면 하단에 "저장한 장소 보기 →" 링크(`href="/saved"`)가 렌더된다 (실제 이동 확인은 `/saved` 페이지가 생기는 Task 4에서 검증)
...
- **수용 기준**:
  - [x] 예산 이내 장소 A(1인 25,000원)와 예산 초과 장소 B(1인 40,000원)가 후보일 때, 예산 30,000원 기준으로 A가 B보다 항상 먼저 나온다
  - [x] 후보가 6곳 이상인 지역 → 결과가 정확히 5곳으로 잘린다
  - [x] 후보 전원이 예산 초과인 지역·예산 조합 → 5곳 모두 `withinBudget: false`로 반환되고, 예산에 가장 가까운 순으로 정렬된다
  - [x] 시드 장소가 3곳뿐인 지역 → 정확히 3곳만 반환된다 (5곳을 채우려고 다른 지역 장소를 섞지 않는다)
  - [x] 존재하지 않는 지역으로 조회 → 빈 배열을 반환한다
...
- **수용 기준**:
  - [x] 지역="강남역", 인원수=8, 예산=30000으로 접속 → 카드가 정확히 5개(시드가 5곳 이상일 때) 표시된다
  - [x] 각 카드에 장소명·카테고리·평점·리뷰수·조회수·1인 예상 비용이 표시된다
  - [x] `withinBudget: false`인 카드에는 "예산 초과" 배지가 보인다
  - [x] 예산 이내 후보가 0곳인 조회 → "예산에 맞는 장소가 없어 가까운 순으로 보여드려요" 문구가 카드 목록 위에 나타나고, 카드는 5개 그대로 렌더링된다 (Scenario 3)
  - [x] 시드 장소가 3곳뿐인 지역으로 조회 → 카드가 정확히 3개 렌더링된다 (Scenario 4)
  - [x] 카드 체크박스를 클릭하면 선택 개수 표시가 즉시 갱신된다 (서버 왕복 없음)
...
- [x] 홈에서 지역·인원수·예산 입력 → 결과 화면에 추천 카드가 뜨는 흐름이 브라우저에서 end-to-end로 동작 (스크린샷 `evidence/checkpoint-1.png`)
...
- **구현 대상**:
  ...
  - `app/saved/page.tsx`
- **수용 기준**:
  - [x] 카드 2개 체크 후 "선택 저장" 클릭 → "2곳을 저장했어요" 안내가 나타난다
  - [x] 저장 목록 화면 진입 → 방금 저장한 2곳이 보인다
  - [x] 장소 여러 건을 저장 → 저장 목록에서 저장 시각 역순(최근 저장이 위)으로 나열된다
  - [x] 유효하지 않은 저장 요청(빈 배열) POST → 400
  - [x] 인증 헤더·쿠키 없이 저장 API를 호출해도 성공한다 (로그인 요구 없음)
  - [x] 저장 레코드에는 저장자를 구분하는 필드가 없다 — 누가 저장했든 같은 저장 목록에 모인다
  - [x] 홈 화면의 "저장한 장소 보기 →" 링크 클릭 → 저장 목록 화면으로 이동한다
...
- **수용 기준**:
  - [x] 결과 화면에서 카드 선택이 0개면 "선택 저장" 버튼이 비활성화되어 클릭 자체가 불가능하다 (spec.md 시나리오 6의 "또는 버튼이 비활성 상태" 분기로 충족 — 별도 경고 토스트는 없음, Task 3에서 이미 구현된 `disabled={selected.size === 0}`를 그대로 사용)
  - [x] 저장 목록에서 장소 A 삭제 → A는 사라지고 다른 장소는 그대로 남는다
  - [x] 인증 헤더·쿠키 없이 삭제 API를 호출해도 성공한다 (로그인 요구 없음)
  - [x] 저장 목록에 3건이 있는 상태에서 "모두 삭제" 확인 → 목록이 0건이 되고 "저장된 장소가 없어요" 문구가 나타난다
...
- **수용 기준**:
  - [x] 추천 결과 5곳으로 공유 텍스트를 만들면 5곳의 이름이 모두 포함된다
  - [x] `navigator.share`가 있는 환경에서 공유 버튼 클릭 → `navigator.share`가 호출된다
  - [x] `navigator.share`가 없는 환경에서 공유 버튼 클릭 → 클립보드에 복사되고 "클립보드에 복사했어요" 안내가 나타난다
...
- [x] 모든 테스트 통과: `bun run test`
- [x] 빌드 성공: `bun run build`
- [x] 저장·삭제·공유가 결과 화면에서 실제로 동작 (스크린샷 `evidence/checkpoint-2.png`)
- [x] web-design-guidelines 기준으로 모바일(375px) 레이아웃에 가로 스크롤이 없는지 확인
...
- **수용 기준**:
  - [x] 홈에서 입력값 미완성으로 제출 → 에러 문구 확인 → 값 채워 재제출 → 결과 화면에 카드 5개 표시 → 2곳 체크 후 저장 → 저장 목록에서 확인 → 1곳 삭제 → 남은 1곳만 존재 확인, 이 흐름이 하나의 스펙에서 전부 통과한다
