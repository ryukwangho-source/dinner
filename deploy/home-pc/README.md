...
| 루프 | 파일 | 역할 |
|---|---|---|
| 서버 실행 | `run-dinner-hidden.vbs` | `.next` 빌드 산출물로 `next start`를 0.0.0.0:3300에 띄우고, 죽으면 자동 재시작 |
| 자동 배포 | `auto-deploy-hidden.vbs` | 2분마다 `origin/main`을 확인해 새 커밋이 있으면 pull → build → 기존 서버 프로세스 종료 |

역할을 분리한 이유: 자동 배포 루프는 "언제 새로 띄울지"만 결정한다. "어떻게 띄울지"(재시작·백오프·로그 로테이션)는 이미 `run-dinner.ps1`이 하고 있으므로, 배포 루프는 새 빌드가 성공했을 때 포트 3300을 쓰는 프로세스를 종료만 하면 `run-dinner.ps1`이 5초 안에 새 빌드로 다시 띄운다.
