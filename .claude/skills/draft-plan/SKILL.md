...
Task 참조가 "기존 서비스 X를 그대로 베껴 도메인에 맞게 수정한다" 형태면, X의 **후처리 방어 로직 목록**(업종·카테고리 화이트리스트, 평점·리뷰수 등 품질 문턱, null/undefined 가드, 상·하한 검증 등)을 plan.md에 명시적으로 나열하고, 새 Task의 수용 기준에 "각 방어 로직이 새 서비스에도 동일하게 적용됐는지" 검증하는 항목을 하나씩 대응시킨다. 프롬프트·파싱·이원 경로 같은 눈에 띄는 구조만 옮기고 후처리 필터는 빠뜨리기 쉽다 — X의 테스트 데이터가 항상 깨끗하면 기존 테스트로도 공백이 드러나지 않는다(venue-manual-course-one 사례: `venue-generator.ts`의 `ALLOWED_CATEGORIES`·`RATING_MIN` 필터가 courseTwo 변환에서 누락됐다가 code-reviewer가 잡음).
